  ...
  $count=0;
	...
	echo "<ul style='display: block;'>";
     while($valOf=mysql_fetch_array($find)){
      $count++;
       echo"<li onclick='homeUniSearchMain3(this.id)' id='mainUnili3".$count."'>"."<span class='nn'>".$valOf['school_name']."</span>"."<span class='main'>".$valOf['school_name']."(".$valOf['school_nick_name'].")"."</span>"."<span class='sub'>".$valOf['city_name'].",".$valOf['state_name']."</span>". "</li>";
     }
     exit;
     echo "</ul>";
   }
...
/*if searching prof according to uni not for rating*/
   if (isset($_REQUEST["profName"]) && isset($_REQUEST["uniNam"]) ) {
     ...
     echo "<ul style='display: block;'>";
       while($valOf=mysql_fetch_array($find)){
         $count++;
         echo"<li onclick='homeFronSearch(this.id)' id='mainProfli".$count."'>"."<span class='main'>".$valOf['first_name']."</span>"."<span class='sub'>".$valOf['first_name'].",".$valOf['last_name']." At ".$valOf['school_name']."</span>". "</li>";
       }
       exit;
     echo "</ul>";
   }
